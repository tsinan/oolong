		...
		MessageSourceAware, InitializingBean
{
	...
	private static final Logger log = LoggerFactory
			.getLogger(DefaultRestErrorResolver.class);
	...
	@Override
	public RestError resolveError(ServletWebRequest request, Object handler,
			Exception ex)
	{
	...
	protected int getStatusValue(RestError template, ServletWebRequest request,
			Exception ex)
	{
		return template.getStatus().value();
	}

	protected int getCode(RestError template, ServletWebRequest request,
			Exception ex)
	{
	...
	protected String getMoreInfoUrl(RestError template,
	...
	protected String getMessage(RestError template, ServletWebRequest request,
			Exception ex)
	{
		return getMessage(template.getMessage(), request, ex);
	}

	protected String getDeveloperMessage(RestError template,
			ServletWebRequest request, Exception ex)
	{
	 ...
	 *         {@code null} if no status message should be returned.
	 */
	protected String getMessage(String msg, ServletWebRequest webRequest,
			Exception ex)
	{
		...
		if (msg != null)
		{
			...
			if (messageSource != null)
			{
				...
				{
					locale = localeResolver.resolveLocale(webRequest
							.getRequest());
				}
	...
	private RestError getRestErrorTemplate(Exception ex)
	{
		...
		{
			log.debug("Resolving to RestError template '" + template
	...
	protected Map<String, RestError> toRestErrors(Map<String, String> smap)
	{
		...
		Map<String, RestError> map = new LinkedHashMap<String, RestError>(
				smap.size());
	...
	protected RestError toRestError(String exceptionConfig)
	{
		String[] values = StringUtils
				.commaDelimitedListToStringArray(exceptionConfig);
	...
	private static int getRequiredInt(String key, String value)
	{
		...
		catch (NumberFormatException e)
		{
			String msg = "Configuration element '" + key
					+ "' requires an integer value.  The value "
					+ "specified: " + value;
