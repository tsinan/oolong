 ...
 * the License.
 */
package com.oolong.platform.web.error;
 ...
 * 
 * <ol>
 * <li>Upon encountering an Exception, the configured {@link RestErrorResolver}
 * is consulted to resolve the exception into a {@link RestError} instance.</li>
 * <li>The HTTP Response's Status Code will be set to the {@code RestError}'s
 * {@link com.oolong.platform.web.error.stormpath.spring.web.servlet.handler.RestError#getStatus() status}
 * value.</li>
 * <li>The {@code RestError} instance is presented to a configured
 * {@link RestErrorConverter} to allow transforming the {@code RestError}
 * instance into an object potentially more suitable for rendering as the HTTP
 * response body.</li>
 * <li>The
 * {@link #setMessageConverters(org.springframework.http.converter.HttpMessageConverter[])
 * HttpMessageConverters} are consulted (in iteration order) with this object
 ...
 * canWrite} the object based on the request's supported {@code MediaType}s will
 * be used to render this result object as the HTTP response body.</li>
 * <li>If no {@code HttpMessageConverter}s {@code canWrite} the result object,
 * nothing is done, and this handler returns {@code null} to indicate other
 * ExceptionResolvers potentially further in the resolution chain should handle
 * the exception instead.</li>
 * </ol>
 * 
 * <h3>Defaults</h3>
 ...
 * <td>Converts Exceptions to {@link RestError} instances. Should be suitable
 * for most needs.</td>
 * </tr>
 * <tr>
 * <td>errorConverter</td>
 * <td>{@link MapRestErrorConverter}</td>
 * <td>Converts {@link RestError} instances to {@code java.util.Map} instances
 * to be used as the response body. Maps can then be trivially rendered as JSON
 * by a (configured) {@link HttpMessageConverter HttpMessageConverter}. If you
 * want the raw {@code RestError} instance to be presented to the
 * {@code HttpMessageConverter} instead, set this property to {@code null}.</td>
 * </tr>
 * <tr>
 ...
 * {@link HttpMessageConverter} instances; JSON will be enabled automatically if
 * Jackson is in the classpath. If you want to match the JSON representation
 * shown in the article above (recommended) but do not want to use Jackson (or
 * the Spring's default Jackson config), you will need to
 * {@link #setMessageConverters(org.springframework.http.converter.HttpMessageConverter[])
 * configure} a different JSON-capable {@link HttpMessageConverter}.
 * 
